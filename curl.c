...
static size_t curl_write_cb(void *contents, size_t size, size_t nmemb, void *user_p)
{
...
int curl_load_url(char *url, Buffer *buffer)
{
  ...
  if(res != CURLE_OK) {
    ...
    return -1;
  } else if ((res = curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &http_response_code)) != CURLE_OK) {
